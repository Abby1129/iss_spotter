...
const fetchCoordsByIP = function (ip, callback) {
  const freegoApi = "https://freegeoip.app/json/";
  request(freegoApi + ip, (error, response, body) => {
    if (error) {
      callback(error, null);
      return;
    }
...
const fetchISSFlyOverTimes = function (coords, callback) {
  const flyoverApi = `http://api.open-notify.org/iss-pass.json?lat=${coords.latitude}&lon=${coords.longitude}`;
